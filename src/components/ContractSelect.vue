    ...
    <div class="content">
      <div v-for="(item, i) in data" :key="i" @click="itemClick(item)">
        <div class="vendor-name">
          <span>合同号：{{ item.isNo }}</span>
        </div>
        <div class="vendor-name">
          <span>合同类型： {{ item.orderMode }}</span>
        </div>
        <div class="vendor-name">
          <span>收款方式：{{ item.outType }}</span>
        </div>
      </div>
    </div>
  </Scroll>
...
import { findInlandSale } from "@/api/erp.warehouse.js";
  ...
  methods: {
    ...
    loadData() {
      ...
      var param = {
        ...
        productName: this.isNo,
        sort: "",
        sortOrder: "",
      };
      findInlandSale(param).then((res) => {
