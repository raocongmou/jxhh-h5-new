    ...
    <div class="content">
      <div v-for="(item, i) in data" :key="i" @click="itemClick(item)">
        <div class="company-money">
          <span> {{ item.vendorNo1 }} - {{ item.unitName5 }} </span>
          <span> {{ item.coNameC }} </span>
          <span>
            {{ item.unitName1 }} - {{ item.unitName2 }} - {{ item.unitName3 }} -
            {{ item.unitName4 }}</span
          >
        </div>
      </div>
...
import { listVendorByUser } from "@/api/erp.vendor.js";
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      listVendorByUser(param).then((res) => {
