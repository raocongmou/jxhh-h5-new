    ...
    <div class="content">
      ...
      <!-- <div v-for="(item, i) in data" :key="i" @click="itemClick(item)"> -->
        <div class="company-money">
          <span>编号: {{ item.pdtNo1 }} </span>
          <span>中文品名: {{ item.pdtNameC }} </span>
          <span>规格型号: {{ item.specificationC }} </span>
        </div>
...
import { findProductByName } from "@/api/domesticTrade/common";
import { getDefaultOrgCoNo } from "@/utils/user.setting";

export default {
  name: "CompanyGoodsDialog",
  ...
  methods: {
    ...
    loadData() {
      ...
      findProductByName(param).then((res) => {
