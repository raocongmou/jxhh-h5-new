    ...
    <div class="content">
      <div v-for="(item, i) in data" :key="i">
        <div class="company-money">
          <span>出口合同号: {{ item.isNo }} </span>
          <span>业务员: {{ item.employeeName }} </span>
          <span>部门: {{ item.orgName }} </span>
...
import { findInlandSaleItem } from "@/api/exportTrade/document";
import { getDefaultOrgCoNo } from "@/utils/user.setting";

export default {
  name: "InlandSaleGoodsDialog",
  ...
  methods: {
    ...
    loadData() {
      ...
      findInlandSaleItem(param).then((res) => {
