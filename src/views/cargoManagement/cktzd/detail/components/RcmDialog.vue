<template>
  ...
  <div class="tool_bar">
    ...
    <van-field
      ...
      v-if="dialogSearchCriteria1"
      :placeholder="'请输入' + placeholder1"
    />
    <van-button size="small" type="primary" @click="query">搜索</van-button>
  </div>
    ...
    >
      <div v-for="(item, i) in data" :key="i">
        <div class="company-money">
          <li v-for="field in showFields" :key="field.enName">
...
export default {
  ...
  data() {
    return {
      ...
      total: 0,
      pageNum: 0,
      searchCriteria: "",//搜索条件1
      searchCriteria1: "",//搜索条件2
  ...
  methods: {
    ...
    loadData() {
      ...
      if(this.dialogSearchCriteria1) {
        param[this.dialogSearchCriteria1] = this.searchCriteria1;
      }
      dialogApi(param, this.interfacePath).then((res) => {
        this.$emit("handleValueConversion", this.showFields, res.data, (data) => {
