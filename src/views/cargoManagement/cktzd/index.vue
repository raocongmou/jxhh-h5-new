  ...
  >
    <div class="content">
      ...
      >
        ...
        <div class="sign-date">
          <span>调整金额</span>
          <span>{{ getFormatMoney(item.amountD) }}</span>
        </div>
        <!-- 调整数量 -->
        <div class="sign-date">
          <span>调整数量</span>
          <span>{{ Number(item.quantityD).toFixed(2) }}</span>
        </div>
        <!-- 制单日期 -->
        <div class="sign-date">
          <span>制单日期</span>
...
import { getWareHouseList } from "@/api/cargoManagement/cktzd";
    ...
    addOrder() {
      this.$router.push({
        ...
        query: { 
          docEntry: null, 
          edit: "add",
          adtStatusList: JSON.stringify(this.auditStatusList),
