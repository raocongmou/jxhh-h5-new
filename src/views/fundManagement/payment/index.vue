  ...
  >
    <div class="content">
      ...
      >
        ...
        <div class="sign-date">
          <span>应付金额</span>
          <span>{{ item.dueAmountS }}</span>
        </div>
        <!-- 付款日期 -->
        <div class="sign-date">
          <span>付款日期</span>
          <span>{{ item.duePayDate }}</span>
        </div>
        <!-- 付款方式 -->
        <div class="sign-date">
          <span>付款方式</span>
...
import { getPaymentList } from "@/api/fundManagement/payment";
...
import { getSaleType } from "@/utils/erp.base.data";
...
export default {
  ...
  methods: {
    selectDateTime({ startDate, endDate }) {
      this.queryForm.payDateRange = [
        dateFormat(startDate, "yyyy-MM-dd"), 
        dateFormat(endDate, "yyyy-MM-dd")
      ];
      this.queryForm.startPayDate = 
      dateFormat(startDate, "yyyy-MM-dd");
      this.queryForm.endPayDate = 
      dateFormat(endDate, "yyyy-MM-dd");
    },
    ...
    async getMatchDataList() {
      const res = await Promise.all([
        getAuditStatusList({}),
        getCurrencyCodeList({})
      ]);
      // this.matchDataList = {
      //   auditStatusList: res[0].data,
      //   currencyCodeList: res[1].data,
      // }
      this.auditStatusList.push(...res[0].data);
      this.currencyCodeList.push(...res[1].data);
    },
    ///// 计算销售类型
    ...
    loadData() {
      ...
      getPaymentList(this.queryForm).then((res) => {
