    ...
    <div class="content">
      ...
      >
        <div class="is-no-audit-status">
          <span>{{ item.raNo }}</span>
          <span :class="getAuditClassBgc(item.comform)">{{
            item.comform === "Y" ? "已确认" : "未确认" 
          }}</span>
        </div>
        <!-- 付款单位 -->
        <div class="sign-date">
          <span>付款单位</span>
          <span>{{ item.paymentCo }}</span>
        </div>
        <!-- 币种 -->
        <div class="sign-date">
          <span>币种</span>
        ...
        <div class="sign-date">
          <span>收款金额(原币)</span>
          <span>{{ moneyFormat(item.amntFeeSum) }}</span>
        </div>
        <!-- 收款金额(本位币) -->
        <div class="sign-date">
          <span>收款金额(本位币)</span>
          <span>{{ moneyFormat(item.amntFeeASum) }}</span>
        </div>
        <!-- 领用金额 -->
        <div class="sign-date">
          <span>领用金额</span>
        ...
        <div class="sign-date">
          <span>未领余额</span>
          <span>{{ moneyFormat(item.amountYe) }}</span>
        </div>
        <!-- 制单日期 -->
        <div class="sign-date">
          <span>制单日期</span>
          <span>{{ getDate(item.issueDate) }}</span>
        </div>
...
import { list } from "@/api/fundManagement/receiptBillManage";
