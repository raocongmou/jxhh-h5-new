    ...
    <div class="content">
      ...
      >
        ...
        <div class="is-no-audit-status">
          ...
          <span :class="getAuditClassBgc(item.payterm)">
            {{ item.payterm + '天' }}
          </span>
        </div>
        <!-- 银行代码 -->
        ...
        <div class="sign-date">
          <span>买方代码</span>
          <span>{{ item.buyerNo }}</span>
        </div>
        <!-- 买方名称 -->
        <div class="sign-date">
          <span>买方名称</span>
          <span>{{ item.buyerengName }}</span>
        </div>
        <!-- 批复限额 -->
        <div class="sign-date">
          <span>批复限额</span>
          <span>{{ item.quotasum }}</span>
        </div>
        <!-- 限额余额 -->
        <div class="sign-date">
          <span>限额余额</span>
          <span>{{ item.quotabalance }}</span>
        </div>
        <!-- 支付方式 -->
        <div class="sign-date">
          <span>支付方式</span>
        ...
        <div class="sign-date">
          <span>拒收风险赔偿比例</span>
          <span>{{ item.refupuserate }}</span>
        </div>
        <!-- 其它商业风险赔偿比例 -->
        <div class="sign-date">
          <span>其它商业风险赔偿比例</span>
          <span>{{ item.otherrate }}</span>
        </div>
        <div class="tow_column">
          <div class="sign-date1">
            <span>测算时间</span>
            <span>{{ getDate(item.calculatetime) }}</span>
          </div>
          <!-- 销售数量 -->
          <div class="sign-date2">
            <span>生效日期</span>
            <span>{{ getDate(item.effecteDate) }}</span>
          </div>
        </div>
      </div>
    </div>
...
import { getQuotaBalanceList } from "@/api/sinosure/quotaBalance";
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      getQuotaBalanceList(this.queryForm).then((res) => {
