<template>
  <div>
    ...
    <div class="tools-bar" v-if="isShowToolBar">
    <!-- <div class="tools-bar" v-if="btnShow.save&&btnShow.cancel&&btnShow.edit"> -->
      <van-button
        v-show="btnShow.save"
        size="small"
      ...
      >
      <van-button
        v-show="btnShow.edit && editStatus === 'view'"
        size="small"
        class="tools-bar-button"
        type="primary"
        color="#1fa2e8"
...
export default {
  ...
  created() {
    provide("title", "采购询价回填详情");
    console.log("this.$route.query", this.$route.query.docEntry);
    this.docentry = JSON.parse(this.$route.query.docEntry);
  ...
  methods: {
    ...
    mainBtnClickCancel() {
      Dialog.confirm({
        ...
        .then(() => {
          if (this.oldFormData && this.oldFormData.docEntry) {
    ...
    mainSelecteEmployee() {
      if (this.orderReadOnly) {
        Notify({ type: "warning", message: "当前页面只允许修改商品采购价格,其他均不可编辑!" });
        return;
      }
