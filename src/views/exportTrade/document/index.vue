  ...
  >
    <div class="content">
      ...
      >
        ...
        <div class="sign-date">
          <span>制作日期</span>
          <span>{{ getDate(item.invoiceDate) }}</span>
        </div>
        <!-- 客户名称 -->
        <div class="sign-date">
          <span>客户名称</span>
        ...
        <div class="sign-date">
          <span>发票类型</span>
          <span>{{ item.invoiceType === 1 ? "议付"
                  : item.invoiceType === 2 ? "报关"
                  : "议付+报关"
           }}</span>
        </div>
        <!-- 币种 -->
...
import { getInvoiceList } from "@/api/exportTrade/document";
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      getInvoiceList(this.queryForm).then((res) => {
