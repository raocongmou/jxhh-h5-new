    ...
    <div class="content">
      ...
      >
        <div class="is-no-audit-status">
          ...
          <span :class="getAuditClassBgc(item.statusFlag1)">
            {{ item.statusFlag1 }}
          </span>
        </div>
        <!-- 客户名称 -->
        ...
        <div class="sign-date">
          <span>开证金额</span>
          <span>{{ getMoney(item.lcamount) }}</span>
        </div>
        <!-- 部门 -->
        <!-- <div class="sign-date">
          <span>部门</span>
...
import { getLcdocList } from "@/api/exportTrade/creditLetter";
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      getLcdocList(this.queryForm).then((res) => {
