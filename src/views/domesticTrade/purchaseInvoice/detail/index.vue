  ...
  <div>
    ...
    <van-tabs v-model:active="tabActive" @change="onClickTab">
      ...
      <van-tab title="入库单核销">
        ...
        >
          <van-collapse-item
            v-for="(item, index) in formData.warehouseItems"
            :key="index"
            :name="index + 1"
            :title="item.pdtName"
          >
...
import {
  getInvoiceInfo,
} from "@/api/domesticTrade/purchaseInvoice";
import { getTable3XByDoc, getTable3XByRef, getAuditStatusList } from "@/api/common";
...
export default {
  ...
  data() {
    return {
      invoiceTypeList: [
        { refValue: "增值税发票", refValueCode: 'I' },
        { refValue: "普通发票", refValueCode: 'C' },
      ],
      heightIndex: "20px",
      auditStatusList: [],
      objectNo: "PM03",
  ...
  methods: {
    getQuantity() {
      return Number(this.formData.quantity).toFixed(2)
    },
    formatterDate(val) {
      if(val) {
    ...
    changeInvoiceType({ label, value }) {
      this.formData.invoiceType = label;
    },
    /// 设置 按钮的状态
    setBtnStatus() {
    ...
    formatMoney(data) {
      data['totalPrice'] = moneyFormat(data['totalPrice']);
    },
    ///重新加载所有的数据
    loadDetail() {
      getInvoiceInfo({ docNum: this.docentry })
      .then((res) => {
        this.formatMoney(res.data);
        if(res.data.items) {
  ...
  computed: {
    formatContrlD() {
      ...
      this.businessTypeList.forEach(item => {
        if(this.formData.contrlD === item.id) {
          text = item.text;
        }
      });
      return text;
