    ...
    <div class="content">
      ...
      >
        ...
        <div class="vendor-name">
          <span>{{ item.vendorName1 }}</span>
        </div>
        <div class="employee-code-total">
          <span>{{ item.contrLdName }}</span>
          <span>{{ item.flag2Name }}</span>
...
import ErpQueryPoInvoice from "@/components/ErpQueryPoInvoice.vue";
import { list } from "@/api/erp.poinvoice.js";
...
export default {
  name: "InlandSaleList",
  components: { NavBar, Scroll, ErpQueryPoInvoice },
  data() {
    return {
      longClickActions: [],
      data: [],
  ...
  methods: {
    ...
    dispatchDetail(item) {
      ...
      this.$router.push({
        ...
        query: { poInvoiceNo: item.poInvoiceNo },
      });
    },
    ///   用户长按 的选择框
