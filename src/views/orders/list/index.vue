<template>
  ...
  <van-action-sheet
    ...
    :close-on-click-overlay="overlay"
  >
    <search :queryForm="queryForm"></search>
  </van-action-sheet>
...
export default {
  ...
  methods: {
    ...
    createDataLongClickActions(item) {
      ...
      // }
      this.longClickActions[0] = { name: "合同预览" };
      this.longClickActions[1] = { name: "查阅" };
    ...
    longClickAction(action) {
      ...
      if (action.name === "查阅") {
        ...
        console.log("查看OA进度");
      } else if (action.name === "合同预览") {
        //console.log("删除", this.longClickTargetItem);
        this.orderPdfView(this.longClickTargetItem, this.longClickTargetIndex);
    ...
    //
    orderPdfView(item, index) {
      let query = {
        keyValue: item.orderSId,
        keyName: "orderSId",
        moduleName: "d_sm07_jxhh",
      };
      this.$router.push({
        path: "/pdforder",
        query,
