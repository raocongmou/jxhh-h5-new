      ...
      <van-tab title="联系信息">
        ...
        >
          <van-collapse-item
            ...
            :title="
              item.controla.trim() === 'B' ? '我方联系信息' : '客户联系信息'
            "
          >
            <van-form>
              <van-field
                ...
                :readonly="orderReadOnly"
                name="公司名称"
                :label="item.controla.trim() === 'B' ? '公司名称' : '客户名称'"
                type="textarea"
                autosize
                rows="1"
              />
...
export default {
  ...
  methods: {
    // 待处理
    computedFormula(row, type) {
      if (
        type === "quantity" 
      ) {
        if (row.sapunitrate && row.sapunitrate !== 0) {
        row.sapqtty = row.quantity / row.sapunitrate;}
        if ( row.quantity !== 0) {
          if (row.unitprice ===0 && row.subprice && row.subprice !== 0) {
            row.unitprice = Number(row.subprice / row.quantity).toFixed(2);
            if ( this.infoForm.exchRate !== 0) {
              row.unitpricea = Number(row.unitprice /  this.infoForm.exchRate).toFixed(2);
            }  
            if (row.sapunitrate && row.sapunitrate !== 0) {
              row.sapprice = Number(row.unitprice * row.sapunitrate * this.infoForm.exchangerate).toFixed(2)
            }
          } else {
            row.subprice = Number(row.unitprice * row.quantity).toFixed(2);
            if ( this.infoForm.exchRate !== 0) {
              row.subpricea = Number(row.subprice /  this.infoForm.exchRate).toFixed(2);
            }  
          }
        }          
      }
      ...
      if (
        ...
        if (row.quantity !== 0) {
          row.unitprice = Number(row.subprice / row.quantity).toFixed(2);
          if ( this.infoForm.exchRate !== 0) {
            row.unitpricea = Number(row.subprice /  this.infoForm.exchRate).toFixed(2);
          }          
        }
        if (row.sapunitrate && row.sapunitrate !== 0) {
          row.sapqtty = row.quantity / row.sapunitrate;
      ...
      this.computeTotal();
      this.computeProfit(row);
    },
    computeProfit(row){
      row.yfrmb = row.yf * this.infoForm.exchangerate;
      row.bxfrmb = row.bxf * this.infoForm.exchangerate;
      row.yjrmb = row.yj * this.infoForm.exchangerate;
      row.qtwbrmb = row.qtwb * this.infoForm.exchangerate;
      if (row.quantity != 0){
        row.lxlx = Number(row.unitstdcost * row.quantity).toFixed(2) * this.infoForm.tbjc * 0.01 * this.lx * 0.01 / 365 * this.infoForm.refNoTitle2;
        row.ts = (row.subprice - row.yf - row.bxf) * this.infoForm.exchangerate * 0.01 * row.dtra;
        row.profitrate = 100 * (row.subprice * this.infoForm.exchangerate - row.unitstdcost * row.quantity - row.yjrmb - row.yfrmb - row.bxfrmb - row.qtwbrmb - row.jcf - row.djz - row.zxf - row.bzf - row.gzf - row.lxlx - row.qt + row.ts - row.subprice * this.infoForm.exchangerate * 0.01 * this.infoForm.tbjc * 0.01 * this.infoForm.tbrate) / (row.subprice * this.infoForm.exchangerate);
        if (this.orderType === "转口") {
          row.unitstdprice = (row.unitstdcost * row.quantity + row.yfrmb + row.bxfrmb + 1 * row.jcf + 1 * row.djz + 1 * row.zxf + 1 * row.bzf + 1 * row.gzf + 1 * row.lxlx + 1 * row.qt + (0.01 * row.dtra * (row.yfrmb + row.bxfrmb) + row.qtwbrmb + row.yjrmb)) / (1 - 0.01 * this.infoForm.tbjc * 0.01 * this.infoForm.tbrate + row.dtra * 0.01 - row.profitrate * 0.01 * 0.9 ) /row.quantity /this.infoForm.exchangerate;
          row.sapunitstdcost = Number(row.unitstdprice * row.sapunitrate * this.infoForm.exchangerate).toFixed(2);    
        }
      }      
    },
    computedInfoForm() {
      // this.formData.items.forEach(item => {
      //   this.computeProfit(item);
      // })

      this.formData.tbamount = this.formData.tbjc * 0.01 * this.formData.totalPrice * this.formData.exchangerate;
      this.formData.tbfee = this.formData.tbrate * 0.01 * this.formData.tbamount;
    },
    collapseChange(name) {
      if (name === 3) {
        this.$nextTick((_) => {
    ...
    loadDetail() {
      detail({ orderSId: this.orderSId })
        .then((res) => {
          ...
          res.data.rmbTotalPrice = moneyFormat(res.data.rmbTotalPrice);
          res.data.totalPriceA = moneyFormat(res.data.totalPriceA);
          res.data.tbamount = moneyFormat(res.data.tbamount);
          // res.data.tbfee = moneyFormat(res.data.tbfee);
          res.data.exchangerate = Number(res.data.exchangerate).toFixed(4);
          ...
          this.formData.tbfee = Number(this.formData.tbrate * 0.01 * rMoneyFormat(this.formData.tbamount)).toFixed(2)
  ...
  computed: {
    ...
    computedContactItems: function () {
      return this.formData.contactItems.filter(function (item) {
        //只返回id是偶数的
        return item.controla.trim() === "B" || item.controla.trim() === "C";
      });
    },
    // 计算高度
