    ...
    <div class="content">
      ...
      >
        ...
        <div class="tow_column">
          <div class="sign-date1">
            <span>是否海外</span>
            <span>{{ item.type6 === "A" ? "境内" : "境外" }}</span>
          </div>

          <div class="sign-date2">
            <span>公司类型</span>
            <span>{{ computerType(item) }}</span>
          </div>
        </div>
        ...
        <div class="tow_column">
          <div class="sign-date1">
            <span>申请日期</span>
            <span>{{ dateFormat1(item.ContactDate) }}</span>
          </div>
          <!-- 销售数量 -->
          <div class="sign-date2">
            <span>申请人</span>
...
import ErpQueryVendor from "@/components/ErpQueryVendor.vue";

import { list, deleteVd05, apply, applySap } from "@/api/erp.vendor.js";
...
export default {
  name: "InlandSaleList",
  components: { NavBar, Scroll, ErpQueryVendor },
  data() {
    return {
      longClickActions: [],
      saleTypeList: [],
  ...
  methods: {
    ...
    computerType(item) {
      if (item.type1 === "Y") {
        return "客户";
      } else if (item.type2 === "Y") {
        return "供应商";
      } else if (item.type3 === "Y") {
        return "物流公司";
      } else if (item.type13 === "Y") {
        return "仓库";
      } else if (item.type4 === "Y") {
        return "保险公司";
      } else if (item.type5 === "Y") {
        return "集团往来";
      } else if (item.type10 === "Y") {
        return "后勤往来";
      } else if (item.type8 === "Y") {
        return "个人往来";
      } else if (item.type9 === "Y") {
        return "银行";
      } else if (item.type11 === "Y") {
        return "协作方";
      } else if (item.type14 === "Y") {
        return "店铺";
      } else if (item.type7 === "Y") {
        return "其他";
      }
      return "";
    },
    ...
    deleteOrder(item, index) {
      ...
      Dialog.confirm({
        title: `确定删除当前往来单位吗?`,
        message: `往来单位【${item.docno}】一旦删除，则无法恢复!`,
      })
        .then(() => {
          deleteVd05({ idList: item.docno }).then((res) => {
            if (res.code === 1) {
