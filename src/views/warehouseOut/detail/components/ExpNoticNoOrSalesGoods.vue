...
import { findInvoiceItemSelect } from "@/api/erp.isinland";

export default {
  name: "ExpNoticNoOrSalesGoods",
  ...
  methods: {
    // 完成按钮处理事件
    commitSelect() {
      this.$emit("handleMutiSelect", this.selectionsArr);
    },
    ...
    loadData() {
      ...
      console.log("ckForm5555", this.ckForm);
      let params = {
        orgCoNo: this.ckForm.orgCoNo,
        // orgCoNo: getStorage("loginInfo").orgCoNo,
        pageSize: 20,
        orderType: this.ckForm.orderType,
        pageNum: this.pageNum,
        invoiceNo: this.ckForm.invoiceNo
      };
      
      findInvoiceItemSelect(params).then((res) => {
