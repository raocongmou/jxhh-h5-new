      ...
      <div v-for="(item, i) in data" :key="i">
        ...
        <div class="multi-checkbox">
          <van-checkbox
            ...
            @click="selectChecked(item.checked, item)"
          />
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    selectChecked(checked, item) {
      ...
      if (item.checked) {
        ...
        this.selectionsArr.splice(
          this.selectionsArr.findIndex((iten) => iten.oid === item.oid),
          1
        );
      }
    },
  ...
  setup(props) {
    ...
    provide(
      ...
      computed(
        () =>
          `选择${
            props.objectNo3 === "SD01"
              ? "出口发票"
              : props.objectNo3 === "SM07"
              ? "转口销售"
              : ""
          }商品`
      )
